...
class SkinCancerDataset(Dataset):
    def __init__(self, metadata_path, image_dirs, transform=None):
...
def main():
    ...
    if not metadata_path.exists():
        ...
        print("Please download and extract HAM10000 dataset first")
        return
    
    full_dataset = SkinCancerDataset(metadata_path, image_dirs, transform=None)
    ...
    class SubsetDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            img = Image.open(self.dataset.image_paths[real_idx]).convert('RGB')
            label = self.dataset.labels[real_idx]
            if self.transform:
                img = self.transform(img)
            return img, label
    
    train_dataset = SubsetDataset(full_dataset, train_indices, get_transforms('train'))
    val_dataset = SubsetDataset(full_dataset, val_indices, get_transforms('val'))
    test_dataset = SubsetDataset(full_dataset, test_indices, get_transforms('test'))
    ...
    # Paper specifies LR=0.2 which is unusually high for Adam; using 1e-4 for stability
    ...
    test_loss, test_acc, test_prec, test_rec, test_f1, preds, labels = validate(model, test_loader, criterion)
...
#%% Grad-CAM Visualization (run after training)
# Example usage - replace with actual image path from test set
# visualize_gradcam(model, 'path/to/image.jpg', get_transforms('test'), 'gradcam_output.png')
