...
def get_transforms(phase):
    if phase == 'train':
        return transforms.Compose([
            ...
            transforms.RandomRotation(25, interpolation=transforms.InterpolationMode.NEAREST),
            transforms.RandomAffine(degrees=0, translate=(0.15, 0.15), shear=15, interpolation=transforms.InterpolationMode.NEAREST),
            transforms.RandomHorizontalFlip(p=0.5),
            transforms.RandomVerticalFlip(p=0.5),
            transforms.ColorJitter(brightness=(0.8, 1.5)),
            transforms.RandomResizedCrop(224, scale=(0.6, 1.0), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.Resize((192, 256), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ])
    if phase == 'val':
        # IMPROVED: no augmentations for validation, only on training data  
        return transforms.Compose([
            transforms.Resize((192, 256), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ...
    if phase == 'test':
        # IMPROVED: also normalize test images
        return transforms.Compose([
            transforms.Resize((192, 256), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
def visualize_gradcam(model, image_path, transform, save_path=None):
    ...
    img_resized = img.resize((256, 192))  # PIL.Image.resize takes (width, height)
...
def main():
    ...
    # IMPROVED: Basically we're doing it good now (I think): This prevents data leakage where same lesion appears in train and test.
    ...
    # IMPROVED: use batch size 32 instead of 8 as in paper
    BATCH_SIZE = 32
    ...
    # IMPROVED: use lr=2e-4 instead of 0.2 as in paper
    ...
    for epoch in range(100):
        ...
        # reactive scheduler: based on val loss
    ...
    axes[0].set_title('Training and Validation Loss — Improved MobileNetV3')
    axes[0].legend()
    axes[1].plot(history['train_acc'], label='Train')
    axes[1].plot(history['val_acc'], label='Val')
    axes[1].set_title('Training and Validation Accuracy — Improved MobileNetV3')
    axes[1].legend()
    plt.tight_layout()
...
def evaluate_saved_model():
    """Load best_model_improved.pth and generate all results needed for a report."""
    ...
    model = ImprovedMobileNetV3(num_classes=7, dropout=0.1).to(device)
    model.load_state_dict(torch.load('best_model_improved.pth', map_location=device, weights_only=True))
    model.eval()
    ...
    plt.title('Confusion Matrix — Improved MobileNetV3')
