...
# TODO: Check whether not normalizing testing data with imagenet stats is possible
def get_transforms(phase):
    ...
    if phase == 'val':
        # As per paper, validation uses same transforms as training
        return transforms.Compose([
            transforms.Resize((224, 224), interpolation=transforms.InterpolationMode.NEAREST),
            transforms.RandomRotation(25, interpolation=transforms.InterpolationMode.NEAREST),
            transforms.RandomAffine(degrees=0, translate=(0.15, 0.15), shear=15, interpolation=transforms.InterpolationMode.NEAREST),
...
def main():
    ...
    # (matching the paper's approach which doesn't mention patient-level splitting)
    ...
    axes[0].set_title('Training Loss — Reproduced MobileNetV3 (Lihore et al., 2024)')
    axes[0].legend()
    axes[1].plot(history['train_acc'], label='Train')
    axes[1].plot(history['val_acc'], label='Val')
    axes[1].set_title('Training Accuracy — Reproduced MobileNetV3 (Lihore et al., 2024)')
    axes[1].legend()
    plt.tight_layout()
...
def evaluate_saved_model():
    ...
    plt.title('Confusion Matrix — Reproduced MobileNetV3 (Lihore et al., 2024)')
    ...
    print("="*60)
    save_dir = 'gradcam_images_original'
    os.makedirs(save_dir, exist_ok=True)
    for i, class_name in enumerate(CLASSES):
        ...
        for img_dir in image_dirs:
            ...
            if img_path.exists():
                ...
                visualize_gradcam(model, str(img_path), get_transforms('test'), f'{save_dir}/gradcam_{class_name}.png')
                break
